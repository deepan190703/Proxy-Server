...
sendErrorMessage (int socket, int status_code)
{
  ...
  strftime (currentTime, sizeof (currentTime), "%a, %d %b %Y %H:%M:%S %Z",
	    &data);
  switch (status_code)
    {
    case 400:
...
connectRemoteServer (char *host_addr, int port_num)
{
  ...
  if (connect
      (remoteSocket, (struct sockaddr *) &server_addr,
...
int
handle_request (int clientSocket, struct ParsedRequest *request, char *buf,
		char *tempReq)
{
  strcpy (buf, "GET ");
  ...
  if (ParsedRequest_unparse_headers
      (request, buf + len, (size_t) MAX_BYTES - len) < 0)
    {
...
main (int argc, char *argv[])
{
  ...
  int reuse = 1;
  if (setsockopt
      (proxy_socketId, SOL_SOCKET, SO_REUSEADDR, (const char *) &reuse,
       sizeof (reuse)) < 0)
  ...
  // Binding the socket
  if (bind
      (proxy_socketId, (struct sockaddr *) &server_addr,
